...
# When each press happens, relative to narration beats (see story/acts/act01.yaml).
PRESS_AT = ["b2+0.55", "b3+0.9", "b4+0.1", "b4+1.25"]


def build(ctx):
    fps, scene, rig = ctx.fps, ctx.scene, ctx.rig
    times = [ctx.beat(r) for r in PRESS_AT]
    player.play(rig, ctx.stream, times, fps, show_signal=False, hold=0.5)

    kx, ky, kz = L.key_pos("L")
    key_cam, key_tgt = S.camera("CAM_key_macro", lens=100, fstop=2.2)
    lamp_cam, lamp_tgt = S.camera("CAM_lamp_macro", lens=85, fstop=2.8)
    ...
    # Lamps: frame the whole lampboard so different lamps are comparable.
    S.move(lamp_cam, lamp_tgt, 0.0, fps, loc=(0.0, -0.22, 0.32), look=(0.0, -0.012, L.LAMP_Z))
    ...
    for i, t in enumerate(times):
        ...
        S.cut(scene, key_cam, min(nxt - 0.25, t + 1.1), fps)
